...
**Styling.** Everything is global CSS in `src/index.css` using CSS custom properties (`--purple`, `--glow`, `--card`, the per-section accent tokens above, etc.) — no CSS modules, no styled-components. Montserrat is pulled in via `@import` at the top of that file. Checkbox spring, strike-through (`scaleX` on `.strike`), and section/list transitions use `framer-motion` (`AnimatePresence` for enter/exit); plain hover/focus states use ordinary CSS transitions.
